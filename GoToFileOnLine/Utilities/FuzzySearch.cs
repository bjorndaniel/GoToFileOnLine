//Adapted from https://github.com/microsoft/vscode/blob/01df559c184549248fcfbcecc5a616462e00c252/src/vs/base/common/fuzzyScorer.ts
/*---------------------------------------------------------------------------------------------
 *  Copyright (c) Microsoft Corporation. All rights reserved.
 *  Licensed under the MIT License. See License.txt at https://github.com/microsoft/vscode for license information.
 *--------------------------------------------------------------------------------------------*/
using System.Collections.Generic;

namespace GoToFileOnLine.Utilities
{
    public static class FuzzySearch
    {
        public static (int score, List<int> positions) ScoreFuzzy(string target, string query, string queryLower)
        {
            ...
            var (score, positions) = DoScoreFuzzy(query, queryLower, queryLength, target, targetLower, targetLength, true);

            return (score, positions);
        }

        private static (int score, List<int> positions) DoScoreFuzzy(string query, string queryLower, int queryLength, string target, string targetLower, int targetLength, bool allowNonContiguousMatches)
        {
            var scores = new int[targetLength * targetLength];
            var matches = new int[targetLength * targetLength];
            for (int queryIndex = 0; queryIndex < queryLength; queryIndex++)
            {
                ...
                for (int targetIndex = 0; targetIndex < targetLength; targetIndex++)
                {
                    ...
                    var score = 0;
                    if (diagScore < 1 && queryIndexGtNull)
                    {
                        score = 0;
                    }
                    else
                    ...
                    if (isValidScore && (allowNonContiguousMatches || queryIndexGtNull || (targetIndex >= queryLower.Length && targetLower.Substring(0, targetIndex).StartsWith(queryLower))))
                    {
            ...
            var queryIndexDone = queryLength - 1;
            var targetIndexDone = targetLength - 1;
            while (queryIndexDone >= 0 && targetIndexDone >= 0)
            {
                var currentIndex = queryIndexDone * targetLength + targetIndexDone;
        ...
        private static int ComputeCharScore(char queryCharAtIndex, char queryLowerCharAtIndex, string target, string targetLower, int targetIndex, int matchesSequenceLength)
        {
            var score = 0;
            ...
            else
            {
                ...
                var separatorBonus = ScoreSeparatorAtPos((CharCode)Enum.Parse(typeof(CharCode), ((int)target[targetIndex - 1]).ToString()));
                if (separatorBonus > 0)
                {
        ...
        private static int ScoreSeparatorAtPos(CharCode code) =>
            code switch
            {
                var res when
                res == CharCode.Slash ||
                res == CharCode.Backslash => 5, // ...over other separators
                var res when
                        res == CharCode.Underline ||
                        res == CharCode.Dash ||
                        res == CharCode.Period ||
                        res == CharCode.Space ||
                        res == CharCode.SingleQuote ||
                        res == CharCode.DoubleQuote ||
                        res == CharCode.Colon => 4, // ...over other separators
                _ => 0
            };
